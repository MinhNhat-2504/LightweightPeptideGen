"""
ESM2-Decoder Trainer: Reconstruction (CrossEntropy) loss with ESM2 conditioning.
"""
...
class ESM2DecoderTrainer:
    """
    Training loop for ESM2-Decoder baseline.
    ...
    def __init__(
        ...
        grad_clip: float = 1.0,
        use_amp: bool = True,
        checkpoint_dir: str = 'baselines/checkpoints/esm2gen',
        log_path: str = 'baselines/logs/esm2gen.log',
    ):
        ...
        # Only train decoder parameters (ESM2 encoder is frozen)
        trainable_params = [p for p in model.parameters() if p.requires_grad]
        self.optimizer = torch.optim.Adam(
            trainable_params, lr=lr, betas=(beta1, beta2), weight_decay=weight_decay
        )
    ...
    def _compute_loss(self, batch):
        ...
        # Get raw string sequences for ESM2 (if batch contains them)
        sequences = batch.get('sequence', None)
        if isinstance(sequences, list):
            pass  # already a list of strings
        else:
            sequences = None  # will use fallback

        out = self.model(input_ids, condition, target_ids, sequences=sequences)
    ...
    def train(self, num_epochs=100, save_every=10, val_frequency=5):
        train_params = self.model.count_parameters()
        all_params = self.model.count_all_parameters()
        logger.info(
            f"[ESM2-Decoder] Training. "
            f"Trainable params: {train_params:,} | Total: {all_params:,}"
        )
        history = []
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch % val_frequency == 0 or epoch == num_epochs:
                ...
                logger.info(
                    f"[ESM2-Decoder] Ep {epoch:3d}/{num_epochs} | "
        ...
        logger.info("[ESM2-Decoder] Training complete.")
        return history
    ...
    def save_checkpoint(self, filename):
        ...
        logger.info(f"[ESM2-Decoder] Checkpoint: {path}")
